...
import { Hook } from './typings/hooks';
import { FiberRoot } from './typings/FiberRoot';
import { ReactRenderer } from './typings/ReactRenderer';
...
function mountFiberNode(node: Fiber) {
  console.log('mountFiberNode', node);
...
function wat(node: Fiber): void {
  if (Boolean(node.type && node.type.__REACT_UI__)) {
    if (node.memoizedProps !== prevProps) {
...
// Установка атрибутов для html элементов
  ...
  extendStaticObject(Reconciler, {
    ...
    mountComponent: (base: any) => (
      ...
      if (typeof result === 'string') {
        // React 0.14.*
        const resultDomElement: any = createDomFromString(result);
        if (!resultDomElement) {
          return result;
        }
